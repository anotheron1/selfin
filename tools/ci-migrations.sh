...
# обходятся ручки чтения: код PR обязан прочесть данные, записанные кодом main.
#
#   tools/ci-migrations.sh <образ бэка main> <образ бэка PR> <сеятель.mjs>
#
# Локально можно подать один образ дважды — тогда проверяются посев и чтение без новых миграций.
# Поднимает свои контейнеры selfin-ci-db и selfin-ci-app (порт 8099) и убирает их за собой;
# боевую базу и стенд не трогает. Вся логика здесь, а не в workflow, — чтобы гонялась и локально.
#
...
# Строк в каждой таблице схемы public — одним запросом, без списка таблиц в скрипте.
counts() {
...
for ep in "${ENDPOINTS[@]}"; do
  ...
  if [ "$code" != "200" ]; then
    failed+=("$ep → $code: $(head -c 300 "$WORK/body")")
  fi
done

{
  ...
  echo
  echo "Ручек чтения обойдено: ${#ENDPOINTS[@]}, не ответили 200: ${#failed[@]}."
  echo
  echo "| таблица | строк после main | строк после PR |"
  echo "|---|---:|---:|"
  join -t '|' -a 1 -a 2 -e '—' -o 0,1.2,2.2 "$WORK/before" "$WORK/after" \
    | awk -F '|' '{ mark = ($2 == $3) ? "" : " ← изменилось"; printf "| %s | %s | %s%s |\n", $1, $2, $3, mark }'
} | tee -a "${GITHUB_STEP_SUMMARY:-/dev/null}"

if [ ${#failed[@]} -gt 0 ]; then
  echo
  echo "::error::ручки чтения не ответили 200 на данных, записанных кодом main"
  printf '    %s\n' "${failed[@]}"
  docker logs --tail 150 "$APP" 2>&1 | sed 's/^/    /'
  exit 1
fi
