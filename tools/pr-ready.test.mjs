...
// Тринадцатое ревью Codex на #86 (P2): у джобы с матрицей GitHub дописывает к имени проверки значения — «Тесты (node 20)».
test('CI: джоба с матрицей — проверки «имя (значения)» засчитываются за джобу', () => {
  const s = ciState([check('Тесты (node 20)', 'pass'), check('Тесты (node 22)', 'pass')], [need('CI', 'Тесты')]);
  assert.equal(s.ok, true);
});

test('CI: проверка с другим именем, начинающимся так же, джобу не заменяет', () => {
  const s = ciState([check('Тесты линтера', 'pass')], [need('CI', 'Тесты')]);
  assert.equal(s.ok, false);
});
