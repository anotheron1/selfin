...
**Дата:** 2026-05-29
**Статус:** ЧАСТИЧНО ВЫТЕСНЕНА — см. врезку ниже
...
> ## Осторожно: раздел «Frontend и UX» описывает экран, которого больше нет
>
> Пометка поставлена 3 сентября 2026 по итогам блока 0.1 ANO-50.
>
> **Что вытеснено.** Раздел «Frontend и UX» (маршрут и навигация, структура страницы, компоненты,
> ползунки, цветовые зоны, drag UX, форматирование) описывает страницу `/wishlist` со сводным
> графиком и списком item'ов. Такой страницы нет: она переписана в «Примерку»
> (`frontend/src/pages/Wishlist.tsx`, спека `2026-07-18-pocket-sandbox-design.md`). Описанный здесь
> экран уцелел лишь внутри свёрнутой секции «Что с капиталом»
> (`frontend/src/components/sandbox/CapitalWhatIf.tsx`).
>
> **Что осталось в силе.** Модель данных, миграция V18, семантика статусов, инварианты, API
> конверсии и симуляции, жизненный цикл item'а — сверены с кодом и в основном сходятся.
>
> **Где расхождения.** Двенадцать штук перечислены в
> `2026-09-03-ano50-block-0-1-findings.md`, зона «Хотелки». Из них восемь — подозрения на код,
> они воспроизводятся руками на этапе 8 и до тех пор остаются гипотезами.
>
> Сверяться с этим документом постранично нельзя, пока пометка не снята.
...
| `OPEN` | На обсуждении | в секции «Активные» | ~~да~~ **нет** — решение владельца 25.09 (ANO-142): блок «Что с капиталом» открывается той же картиной, что Стратегия; обсуждаемую примеряют галочкой | **нет** | нет |
