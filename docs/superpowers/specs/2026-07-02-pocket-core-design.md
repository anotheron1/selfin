...
`pocket`, `currentBalance`, `minPoint {date, balance}`, `horizon {type, endDate, label, fallback:boolean}`, `buffer`, `trajectory [{date, balance}]`, `breakdown [строки, см. §5]`, `wishlistCandidates [{id, description, plannedAmount, date?, fixed:boolean}]`.
