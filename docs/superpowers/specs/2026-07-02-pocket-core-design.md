...
> **ANO-16 §6 (2026-07-18):** датированные FIXED-неконвертированные **копилки** (`TargetFund`, `purchaseType=SAVINGS`) теперь тоже резервируются в траектории — синтетическими взносами `остаток/n` (строка breakdown `SAVINGS_CONTRIBUTIONS`). Таблица выше описывает только события; правила копилок — в спеке `2026-07-18-pocket-sandbox-design.md` §6.

### 3.3 Алгоритм

1. **Текущий баланс** = checkpoint + Σ фактов (правило 3.2, только записи-факты) с даты чекпоинта по `asOfDate` включительно. Прошлые `PLAN(PLANNED)` в баланс НЕ входят (деньги не потрачены — баланс не убываем). ~~Известная неоднозначность, наследуемая из текущего кода (`>= checkpoint.date` в `calcPocketBalance` и `DashboardService`): факты с датой = дате чекпоинта могут быть уже отражены в его сумме (задвоение); сохраняем семантику для совместимости, решается в ANO-15 (ре-якорь).~~ **РЕШЕНО в ANO-15 (2026-07-18):** сумма чекпоинта = «число из банка на конец его дня», в баланс идут факты СТРОГО ПОСЛЕ даты чекпоинта (см. спеку `2026-07-18-pocket-reanchor-design.md` §5).
...
> **ANO-28 (2026-07-28):** резервируется только просрочка с датой **строго после последнего якоря**. Семантика якоря (ANO-15): «число из банка уже содержит всё до него» — план старше чекпоинта либо исполнен (резерв был бы задвоением с суммой якоря), либо неактуален; живой долг из прошлого переносят на будущую дату. На данных Кирилла это сняло −50 400 ₽ фантомного резерва из марта–мая.
