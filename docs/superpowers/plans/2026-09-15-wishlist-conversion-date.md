...
- [x] **Step 1: Проверочные запросы на рабочей базе ДО выкладки**
...
- [x] **Step 3: Стенд вернуть к эталону**
...
Ветка `fix/ano-138-conversion-date`. Прогон: **400 юнитов, 160 интеграционных, 122 фронтовых**, `tsc` чистый. Одиннадцать мутаций, каждая поймана ровно своим тестом.

## Замеры на стенде

Дефект оказался **не теоретическим**. Проверочный запрос на базе стенда нашёл две живые пары:

| хотелка | артефакт |
|---|---|
| Детское велокресло | событие с пустой датой |
| Рюкзак для фрирайда | событие с пустой датой |

Сирот — ноль, поэтому решение «их не трогаем» ничего не стоило на практике.

`V25` применилась при первом старте (`Successfully applied 1 migration, now at version v25`). После неё: обе хотелки `OPEN` со снятой ссылкой и неудалённые, оба артефакта помечены удалёнными, битых пар ноль.

**Эталон не сдвинулся** — проверено запросом, а не рассуждением:

| | до | после |
|---|---|---|
| капитал | 2 668 277 | 2 668 277 |
| остаток | 60 000 | 60 000 |
| якорь | 29.08 | 29.08 |

Перед миграцией снят дамп базы стенда (356 КБ) — на случай, если бы числа поехали.

## Что нашлось сверх плана

**Дыра в собственном дизайне, которую не поймал ни один тест.** У «Детского велокресла» срок 01.09 — в прошлом (туда его записала примерка, ANO-139). Поле предзаполнялось этим сроком, кнопка была активна, `POST /convert` отвечал 400, и ошибку глотал `.catch(refetch)`. Проверено живьём в сети браузера. Граница в форме приведена к серверной: `min` = завтра, кнопка неактивна, пока срок не строго в будущем. Три новых теста на границу.

**Диалог конверсии живёт за свёрнутым блоком.** Видимые строки на «Примерке» рисует `SandboxItemRow`, и его «Зафиксировать в плане» идёт в `/fix` — путь, где проверка даты стояла всегда. Диалог с конверсией (`/convert`, тот самый дефектный) открывается только из `WishlistItemList` внутри свёрнутого блока «Что с капиталом». Это объясняет, почему дефект нашёлся чтением кода, а не тыком. Смежно с ANO-109, ANO-114, ANO-142.

**ANO-141 подтверждён живьём дважды.** `POST /fix → 400` и `POST /convert → 400`, оба раза экран промолчал.

## Расхождения плана с реальностью

1. **Тест `/fix` не был слабым.** В плане он назван «несущим две приметы» по имени, без чтения тела. В теле три отдельные проверки, любая мутация его красит. Разделение отменено как косметика на чужом зелёном тесте.
2. **Третья мутация Задачи 1 была тождественна оригиналу.** Пока `planDate` нет, проверенная переменная и `src.getDate()` — одно значение. Перенесена в Задачу 2, где сработала.
3. **Ограничение `chk_event_converted_only_fixed` уже снято** миграцией `V22` (починка ANO-103). Довод «иначе схема не даст сменить статус» заменён на настоящий: `ensureNotConverted` отдаёт 409 на непустой ссылке.
4. **Колонка мягкого удаления — `is_deleted`, не `deleted`.** По памяти миграция не применилась бы.
5. **`perl -0pi` с `\n` молча не матчится после `git checkout`** — файл возвращается с CRLF. Поймано обязательным `grep`; для многострочных шаблонов нужен `\r?\n`.
6. **`diff` через rtk соврал** «Files are identical» на файлах с разными хешами. На сравнении файлов прокси верить нельзя.

## Отложено отдельной задачей

**ANO-161** — блок «Хотелки» на Аналитике не смотрит на `wishlist_status`: отклонённые и зафиксированные висят вместе с открытыми. Найдено при разборе, бэклог проверен, дубля нет.
