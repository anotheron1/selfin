...
> **Задачи 2 и 3 слиты при реализации.** Разделить их нельзя: предикат движка обязан сохранить `status == PLANNED` — иначе поедет `factDisplacesPlan`, где исполненный план с непривязанным фактом не должен резервироваться. А раз статус остаётся в предикате, частично погашенный план обязан носить `PLANNED`, то есть правило статуса из Задачи 3 должно приехать тем же коммитом. Иначе движок считает остаток правильно, а в проде план всё равно выпадает по статусу.
>
> **Второе отступление от плана.** Собственный `factAmount` плана в сумму погашенного НЕ складывается. Такие строки (легаси-путь PATCH) носят `EXECUTED` и отсекаются статусом раньше; сложение было бы мёртвым кодом. Вместо этого сохранено прежнее условие `factAmount == null`, а тест переформулирован: легаси-строка по-прежнему не удерживается.
...
- [x] **Step 1: Падающие тесты на остаток**
...
- [x] **Step 3: Добавить `parentEventId` в снимок**
...
- [x] **Step 4: Считать погашенное в движке**
...
- [x] **Step 5: Переписать предикат и суммы**
...
- [x] **Step 6: Зелено, коммит, мутации**

| мутация | результат |
|---|---|
| `remainderOf` возвращает `planned` | **3 падения** |
| сумма будущего дня снова `plannedAmount` | **1 падение**, `partialFact_leavesRemainderReserved` |
| `target` всегда `EXECUTED` | **2 падения**, включая `partialFact_keepsPlanPlanned` |
| `settled.compareTo(planned) > 0` вместо `>=` | **1 падение**, `factCoveringPlan_marksExecuted` |
| убрать `.max(BigDecimal.ZERO)` | **не покраснело — находка, см. ниже** |

Вторая строка важна отдельно: предикат может быть верным, а сумма всё равно браться старая. Проверено — она и правда ловится только собственной мутацией.

**Находка: пол в ноль недостижим поведением.** Снятие `.max(BigDecimal.ZERO)` не покрасило ничего. Причина: при переплате остаток отрицателен, а предикат требует `signum() > 0` — такой план выпадает из траектории раньше, чем отрицательная сумма куда-то попадёт. То есть пол и предикат страхуют друг друга, и ни один тест через движок их не различит.

Оба оставлены сознательно: пол делает `remainderOf` тотальной функцией для любого будущего вызывающего, предикат убирает погашенные планы из списков разбивки. Но покрытие здесь честно неполное, и это записано, а не замолчано.
...
- [x] **Step 2: Считать сумму детей при записи факта**
...
- [x] **Step 3: Проверить потребителей статуса**
...
- [x] **Step 4: `npm test`, `tsc`, коммит, мутация**
...
Ветка `fix/ano-155-plan-remainder`. Прогон: **408 юнитов, 158 интеграционных, 122 фронтовых**, `tsc` чистый. Двенадцать мутаций, одиннадцать поймали своё.

## Замеры на стенде

Будущих планов с привязанными фактами на стенде нет — значит правка не должна была сдвинуть эталон. Проверено замером, а не рассуждением:

| | до | после |
|---|---|---|
| кармашек | −10 100 | −10 100 |
| капитал | 2 668 277 | 2 668 277 |
| остаток | 60 000 | 60 000 |
| якорь | 29.08 | 29.08 |

**Правило проверено живьём.** План 6 000 на +7 дней уронил кармашек ровно на 6 000. Факт 5 480 сегодняшним числом кармашек **не сдвинул** — деньги ушли, 520 продолжают удерживаться, план остался `PLANNED`. Добитый остаток 520 перевёл план в `EXECUTED`, и снова без скачка. Факт завтрашним числом отвергнут с HTTP 400. Стенд возвращён к эталону точь-в-точь.

## Что нашлось сверх плана

**Разорванная пара «@Transactional + метод».** Вставляя `requireNotFuture` перед `createLinkedFact`, я поместил его между аннотацией и сигнатурой — аннотация уехала на хелпер, и запись факта пошла в read-only транзакции класса. Факт не коммитился вовсе. **Ни один из 408 юнитов этого не видит по построению**: они на моках, транзакции там нет. Поймал интеграционный — падением на уборке, а не на утверждении.

**`TaskStop` не убивает форкнутую JVM.** Бэкенд с прошлой проверки (ANO-138) остался слушать 8090 через четыре часа после остановки задачи. Останавливать процесс по порту, а не только задачу.

## Расхождения плана с реальностью

1. **Задачи 2 и 3 слиты.** Предикат обязан сохранить `status == PLANNED` — иначе поедет `factDisplacesPlan`. А раз статус в предикате, правило статуса должно приехать тем же коммитом.
2. **Собственный `factAmount` плана в погашенное не складывается** — легаси-строки носят `EXECUTED` и отсекаются статусом раньше; сложение было бы мёртвым кодом.
3. **Мутация «убрать пол в ноль» не покраснела.** Пол и предикат страхуют друг друга; через движок их не различить. Оставлены оба, покрытие честно неполное.
4. **Текст ошибки оставлен английским** — по соседям; правило 13 про тексты на экране, а эти сообщения человеку пока не показываются (ANO-141).
5. **Правило вынесено в `PlanRemainder`**, а не оставлено «слово в слово» в двух местах. Сторож `PlanRemainderSingleSourceTest` поймал подсаженную вторую копию при зелёных поведенческих тестах — ровно то, ради чего он нужен.
6. **Тесты чистой функции фронта написаны после реализации**, порядок нарушен. Сенситивность доказана мутацией границы. Откатить её по гиту не вышло — файл ещё не был закоммичен, та самая грабля из шапки.

## Переписано по смыслу, не подогнано числом

* `createLinkedFact_success_createsFact` (юнит) и `createLinkedFact_success` (IT) — оба ждали `EXECUTED` от факта, не покрывающего план.
* Три теста `RecurringFactIT` писали факт датой планового платежа, а правило заводится на завтра. Суть их воспроизведения (ANO-91) — факт на порождённом правилом плане, а не его дата.
