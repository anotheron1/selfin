...
**Расхождение с планом.** Текст оставлен английским — `fact date must not be in the future` — по соседям: `requireFutureDate` и прочие 400 в этом коде написаны так же. Правило 13 про тексты НА ЭКРАНЕ, а эти сообщения человеку сейчас не показываются вовсе (ANO-141). Когда начнут — переводить весь слой разом, а не одну строку.

- [x] **Step 2: Прогнать — должны падать**

- [x] **Step 3: Проверка в обоих методах**

Одна приватная проверка на оба входа, «сегодня» из `Clock`. Не две копии: разъедутся при первой же правке.

- [x] **Step 4: Зелено, коммит, мутации**
