...
- [x] **Шаг 4: Прогнать — семь тестов проходят**

- [x] **Шаг 5: Проверить мутациями**
...
- [x] **Шаг 2: Прогнать и убедиться, что первый падает**
...
- [x] **Шаг 3: Перевести factsDelta на правило**
...
- [x] **Шаг 4: Прогнать — оба теста проходят**
...
- [x] **Шаг 1: Переписать тест, закрепивший дефект**
...
- [x] **Шаг 2: Прогнать и убедиться, что второй падает**

- [x] **Шаг 3: Расширить записи**
...
- [x] **Шаг 4: Перевести шаг 1 движка на правило**
...
- [x] **Шаг 3: Перевести дрейф на правило**
...
- [x] **Шаг 4: Прогнать — тест проходит**
...
- [x] **Шаг 2: Прогнать — все три проходят**
...
- [x] **Шаг 1: Полный прогон** — `mvnw verify`, ожидается 350+ юнитов и 142+ интеграционных, ноль падений.
...
- [x] **Шаг 3: Воспроизвести исходный сценарий ANO-125**
...
- [x] **Шаг 4: Проверить, что защита ANO-28 на месте**
...
- [x] **Шаг 7: Отметиться в Linear** — комментарии в ANO-82 и ANO-125 с замерами до и после, списком мутаций и результатом скриптов. Перевести обе в Done. ANO-79 **не закрывать**: в ней остаётся объяснение дельты.

---

## Выполнено 13 сентября 2026

Все шесть задач закрыты. Прогон: **355 юнитов + 142 интеграционных, ноль падений** (до починки 343 + 139). Коммиты: `3ec1a03`, `2593aa0`, `aa9cd30`, `7a0686b`, `135180e`.

### Где реальность разошлась с планом

**Время — `LocalDateTime`, не `Instant`.** Сигнатура в шапке задачи 1 называла `Instant`, но у обеих сущностей `created_at` объявлен как `LocalDateTime`, и сниппеты самого плана тоже. Взят `LocalDateTime`.

**Пятое место, которого в плане нет.** `PocketSandboxService:120` строит производный `PocketInput` из базового. Не прокинув туда `checkpointCreatedAt`, песочница молча вернулась бы к правилу «день якоря целиком не считается» и разошлась бы с `GET /pocket` ровно на факты дня ре-якоря. На том же месте уже стоит предупреждение про 13-арный конструктор — тот же класс ошибки, второй раз.

**Мутация задачи 3 ловится не тем тестом, что ожидал план.** План ждал, что возврат `factCreatedAt == null → true` уронит `factOnCheckpointDay_recordedBefore_notDoubleCounted`. У того теста время записи задано явно, и мутация его не трогает. Добавлен третий тест — `factOnCheckpointDay_unknownEntryTime_staysOut`; без него ветка «нечем решить» осталась бы непроверенной, а на ней сидят 33 остальных теста класса и вся синтетика.

**Два существующих теста сменили контракт, а не сломались.** `AccountBalanceServiceTest.balanceAt_defaultAccount_factSelectionRuleMatchesPocketEngine` и `BalanceCheckpointServiceTest.driftChain` держали факты дня якоря на умолчании `createdAt = now()`. Пока правило смотрело на дату, это ничего не значило; с этой правкой они начали мерить порядок конструирования объектов и момент запуска. Время записи проставлено явно, числа тестов не изменились.

**Порядок шагов 5 и 6 задачи 6 переставлен.** Скрипты откалиброваны под эталонный слепок, а шаг 3 стенд изменяет. Стенд восстановлен до прогона скриптов, иначе они мерили бы оснастку проверки, а не починку.

### Что осталось открытым

**ANO-82 не переведена в Done.** В её описании три механизма, починен первый. Живы «факт с датой позже сегодня не учитывается нигде» и «факт на будущий план уводит расход целиком» — оба часть механизма 2, вынесенного за рамки решением владельца. Отдельной задачи под него нет, её надо завести. ANO-125 закрыта: там механизм был один.

**Расхождения `ano50-stage5.mjs` — не от этой починки.** 39 сошлось, 4 расхождения. Числа стенда после пересборки совпали с дореформенными до копейки, а блоки 5.16/5.17 кладут события на 16.09 и 17.09 при горизонте кармашка 15.09. «Сдвиг 0» здесь значит «событие за горизонтом» — ровно та ловушка, о которой предупреждает передача сессии.
