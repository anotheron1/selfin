...
class PocketMigrationRegressionTest {
     ...
     * Прогноз незапланир. 2 100 — остаток месячной нормы на 11.03..31.03 (21 день). В окно
     *                     11.03..28.03 (18 дней = min(конец марта 31.03, горизонт 28.03)) попадает
     *                     доля 2 100 × 18/21 = 1 800 -> 100/день ровно (без остатка копеек).
     *                     ANO-140: до неё в окно ложилась вся сумма, и эталон подавал «1 800 на окно»;
     *                     сценарий и все числа ниже прежние — поменялся только смысл входа.
     * Буфер       5 000
