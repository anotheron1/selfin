...
describe('buildPocketPhrase', () => {
    ...
    it('НЗ > 0: min = pocket + buffer, НЗ упомянут', () => {
        const p = make({
            pocket: 36000,
            buffer: 5000,
        ...
        expect(buildPocketPhrase(p)).toBe(
            `Свободно ${fmtC(36000)} до дохода 15.07. Самый узкий день — 12.07: на счёте останется ${fmtC(41000)} («Страховка»). НЗ ${fmtC(5000)} уже отложен. После дохода → ${fmtC(129000)}.`,
        );
    });
    ...
    it('НЗ задет: минимум положительный, но ниже НЗ', () => {
        const p = make({
            pocket: -2000,
            buffer: 5000,
        ...
        expect(buildPocketPhrase(p)).toBe(
            `Впритык: в узкий день 12.07 на счёте останется ${fmtC(3000)} — меньше НЗ ${fmtC(5000)}. После дохода → ${fmtC(129000)}.`,
        );
    });
