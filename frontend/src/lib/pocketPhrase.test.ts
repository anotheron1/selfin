...
describe('buildPocketPhrase', () => {
    ...
    it('подушка > 0: min = pocket + buffer, подушка упомянута', () => {
        const p = make({
            pocket: 36000,
            buffer: 5000,
        ...
        expect(buildPocketPhrase(p)).toBe(
            `Свободно ${fmtC(36000)} до дохода 15.07. Самый узкий день — 12.07: на счёте останется ${fmtC(41000)} («Страховка»). Подушка ${fmtC(5000)} уже отложена. После дохода → ${fmtC(129000)}.`,
        );
    });
    ...
    it('подушка прогрызена: минимум положительный, но ниже подушки', () => {
        const p = make({
            pocket: -2000,
            buffer: 5000,
        ...
        expect(buildPocketPhrase(p)).toBe(
            `Впритык: в узкий день 12.07 на счёте останется ${fmtC(3000)} — меньше подушки ${fmtC(5000)}. После дохода → ${fmtC(129000)}.`,
        );
    });
